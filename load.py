...
def run_pipeline():
    ...
    try:
        ...
        print(f"Fetched {rows_fetched} coins from api")
        ...
        if new_coins:
            ...
            print(f"Upserted {rows_loaded} rows")

            newest_timestamp = max(parse_timestamp(c['last_updated']) for c in new_coins)
            update_watermark(PIPELINE_NAME, newest_timestamp)
            print(f"Watermark updated on {newest_timestamp}")
        else:
